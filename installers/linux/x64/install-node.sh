...
# Node.js Installer for Linux x64
...
echo "🚀 Starting Node.js installation for Linux x64..."
...
echo "3. 📥 Downloading Node.js v$target_version for Linux x64..."
echo "   → URL: https://nodejs.org/dist/v$target_version/node-v$target_version-linux-x64.tar.gz"

echo "4. 📦 Extracting Node.js archive..."
echo "   → Extracting to /usr/local/..."

echo "5. 🔗 Creating symbolic links..."
echo "   → ln -sf /usr/local/node-v$target_version-linux-x64/bin/node /usr/local/bin/node"
echo "   → ln -sf /usr/local/node-v$target_version-linux-x64/bin/npm /usr/local/bin/npm"
